...
| `data.js` … `data5.js` | 成语库，共 **371 条**（拼音、四字逐字拼音、中英释义、例句及拼音/英译、配色）。加成语只改这些文件，不必动代码 |
...
- **纸张皮肤**：米黄 / 宣纸 / 夜间（深色）三套，一键切换；皮肤与配色本地记忆。
- **分享**：一键复制今日成语文案（成语+拼音+释义+例句+链接），手机端调起系统分享。
- **随机一条**：手气按钮随机翻到一条成语。
- **收藏**：❤ 收藏喜欢的成语，「浏览」面板里可只看收藏。
- **分类浏览**：☰ 打开浏览面板，按学习进取 / 品格诚信 / 谋略行事 / 团结情谊 / 心境情绪 / 成功成就 / 景物自然 / 才华技艺等分类，或直接搜索成语/拼音/释义。
- 交互：`◀ 前一天` / `后一天 ▶` / `今天`；`← / →` 键翻页、`空格`回今天、手机可左右滑动；按「一年中的第 N 天」稳定轮换。
- 动效：撕历翻页、成语逐字弹出、拆解格悬浮、背景渐变与漂浮圆。
